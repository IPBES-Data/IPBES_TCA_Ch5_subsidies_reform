...
read_corpus("data/corpus") |>
    ...
    dplyr::collect() |>
    write.csv(file = "sent_analysis_subsidies.csv")

read_corpus("data/corpus") |>
    dplyr::select(id, publication_year, ab) |>
    dplyr::collect() |>
    dplyr::filter(id %in% readRDS("data/ids_subs_tca.rds")) |>
    write.csv(file = "sent_analysis_subsidies_tca.csv")
```
...
The number of hits are hits of the terms of the whoel OpenAlex corpus. Due to methodological issues, the number of `R1 AND R2` are overestimates and contain some double counting.
...
- **`R1 AND R2` and in TCA corpus:** `r formatC(length(readRDS("data/ids_subs_tca.rds")), format="f", big.mark=",", digits=0)` hits
...
- works in the subsidies corpus which can be downloded [here](sent_analysis_subsidies.csv){target=_blank}. 
- works in the subsidies corpus AND in the TCA corpus which can be downloded [here](sent_analysis_subsidies_tca.csv){target=_blank}. 
