...
st <- readLines(file.path("input", "ch_5_subsidies_reform", "R1.txt"))[1:70]
st[70] <- gsub(pattern = " OR", replacement = "", st[70])
R1_st[[1]] <- paste(collapse = " ", st)

st <- readLines(file.path("input", "ch_5_subsidies_reform", "R1.txt"))
st <- st[71:length(st)]
R1_st[[2]] <- paste(collapse = " ", st)

rm(st)

R2_st <- readLines(file.path("input", "ch_5_subsidies_reform", "R2.txt")) |>
    paste(collapse = " ")
...
pages_dir <- file.path(".", "data", "ch_5_subsidies_reform", "pages")
arrow_dir <- file.path(".", "data", "ch_5_subsidies_reform", "corpus")
...
arrow_dir <- file.path(".", "data", "ch_5_subsidies_reform", "corpus_tca")
...
ids_subsidies <- read_corpus(file.path("data", "ch_5_subsidies_reform", "corpus")) |>
    dplyr::select(id) |>
    collect() |>
    unlist()

ids_tca <- read_corpus(file.path("..", "IPBES_TCA_Corpus", "data", "ch_5_subsidies_reform", "corpus")) |>
    dplyr::select(id) |>
    collect() |>
    unlist()
...
saveRDS(ids_subs_tca, file = file.path("data", "ch_5_subsidies_reform", "ids_subs_tca.rds"))
```
...
- works in the subsidies corpus AND in the TCA corpus which can be downloded [here](data/random_50_subsidies_in_tca.xlsx){target=_blank}. 
...
```{r}
data <- read_corpus(file.path("data", "ch_5_subsidies_reform", "corpus_tca")) |>
...
![](`r file.path("maps", "ch_5_subsidies_reform", "publications_countries.png")`)
