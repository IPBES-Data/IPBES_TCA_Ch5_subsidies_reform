...
params:
    ...
    #
    # input
    fn_sentiment_results: !expr file.path("input", "ch_5_subsidies_reform", "sensitivity_analysis_output_02_01.csv")
    # data
    fn_count: !expr file.path("data", "ch_5_subsidies_reform", "st_count.rds")
    fn_publications_per_country_data: !expr file.path("data", "ch_5_subsidies_reform", "publications_per_country_data.rds")
    fn_publications_temporal: !expr file.path("data", "ch_5_subsidies_reform", "publications_over_time.rds")
    fn_sentiment_spatial_data: !expr file.path("data", "ch_5_subsidies_reform", "sentiment_spatial_data.rds")
    fn_sentiment_temporal_data: !expr file.path("data", "ch_5_subsidies_reform", "sentiment_temporal_data.rds")
    #
    # figures parameter
...
if (!file.exists(params$fn_sentiment_spatial_data)) {
    data <- read_corpus(params$corpus_authors_dir) |>
        ...
        dplyr::left_join(
            read.csv(params$fn_sentiment_results) |>
                select(
                    work_id = id,
                    neg,
                    neu,
...
if (!file.exists(params$fn_sentiment_temporal_data)) {
    data <- read.csv(params$fn_sentiment_results) |>
        select(
            work_id = id,
            year = date,
            neg,
